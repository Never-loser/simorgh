...
class Strings(val lang: Lang) {
    ...
    val lessons get() = t("آموزش", "Learn")
    val learnOpenings get() = t("گشایش‌ها", "Openings")
    val learnEndgames get() = t("آخربازی", "Endgames")
    val learnPuzzles get() = t("معما", "Puzzles")
    val egMoves get() = t("حرکت‌های شما", "Your moves")
    val egHint get() = t("راهنمایی", "Hint")
    val egHow get() = t("روش کار", "How it is done")
    val egGoalMate get() = t("هدف: مات کن، در حداکثر {n} حرکت.", "Goal: mate, in at most {n} moves.")
    val egGoalPromote get() = t("هدف: پیاده را وزیر کن، در حداکثر {n} حرکت.", "Goal: make a queen, in at most {n} moves.")
    val egGoalHold get() = t("هدف: {n} حرکت دوام بیاور و نگذار پیاده‌ی حریف وزیر شود.", "Goal: last {n} moves without the enemy pawn queening.")
    val egYourMove get() = t("نوبت شماست. موتور با تمام قدرت بازی می‌کند.", "Your move. The engine plays at full strength.")
    val egWon get() = t("آفرین! موفق شدید ✓", "Well done! ✓")
    val egLost get() = t("این بار نشد", "Not this time")
    val egWonMate get() = t("مات کردید.", "You gave mate.")
    val egWonPromoted get() = t("پیاده وزیر شد.", "The pawn became a queen.")
    val egWonHeld get() = t("دوام آوردید؛ پیاده‌ی حریف وزیر نشد.", "You held out; the pawn never queened.")
    val egWonStalemate get() = t("پات شد: مساوی، یعنی دفاع موفق بود.", "Stalemate: a draw, so the defence worked.")
    val egWonNoPawns get() = t("پیاده‌ی حریف از بین رفت: دیگر نمی‌تواند ببرد.", "The enemy pawn is gone: it can no longer win.")
    val egLostMated get() = t("مات شدید.", "You were mated.")
    val egLostPromoted get() = t("پیاده‌ی حریف وزیر شد.", "The enemy pawn queened.")
    val egLostStalemate get() = t("پات شد: شاه حریف حرکتی ندارد و کیش هم نیست، پس بازی مساوی است.",
                                  "Stalemate: the enemy king has no move and is not in check, so it is a draw.")
    val egLostPawn get() = t("پیاده‌ی شما از دست رفت.", "Your pawn was lost.")
    val egLostFifty get() = t("پنجاه حرکت بدون گرفتن مهره یا حرکت پیاده گذشت: مساوی.", "Fifty moves without a capture or a pawn move: a draw.")
    val egLostLimit get() = t("تعداد حرکت‌ها تمام شد.", "Out of moves.")
    val pzRating get() = t("ریتینگ معما", "Puzzle rating")
    val pzPlayed get() = t("معماهای بازی‌شده", "Puzzles played")
    val pzStreak get() = t("درست پشت‌سرهم", "Solved in a row")
    val pzSource get() = t("معماها از پایگاه معمای لیچس (آزاد، CC0) انتخاب شده‌اند و با سطح شما جور می‌شوند.",
                           "Puzzles from the Lichess puzzle database (free, CC0), matched to your level.")
    val pzWhiteToMove get() = t("سفید بازی می‌کند", "White to play")
    val pzBlackToMove get() = t("سیاه بازی می‌کند", "Black to play")
    val pzFind get() = t("بهترین حرکت را پیدا کنید.", "Find the best move.")
    val pzWrong get() = t("این حرکت نیست. دوباره امتحان کنید.", "That is not it. Try again.")
    val pzSolved get() = t("درست! معما حل شد ✓", "Correct! Puzzle solved ✓")
    val pzFailed get() = t("این معما حساب نشد (اشتباه، راهنمایی یا دیدن راه‌حل). بروید سراغ بعدی.",
                           "This one does not count (a wrong move, a hint or the solution). On to the next.")
    val pzPuzzleRating get() = t("ریتینگ این معما", "This puzzle's rating")
    val pzSolution get() = t("نمایش راه‌حل", "Show solution")
    val pzNext get() = t("معمای بعدی", "Next puzzle")
